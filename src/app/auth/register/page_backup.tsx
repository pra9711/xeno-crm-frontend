...
export default function RegisterPage() {
  ...
  const { user, loading: authLoading, refresh, login } = useAuth()
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
      // Use the new auth service method
      const result = await authService.registerWithEmail(email, name, password);
      ...
      if (result.success && result.user) {
        ...
        if (result.error) {
          if (result.error.includes('User already exists')) {
            ...
            show({ message: 'An account with this email already exists' });
          } else if (result.error.includes('stronger password')) {
            setErrors({ password: 'Please choose a stronger password' });
            show({ message: 'Please choose a stronger password' });
          } else {
            setErrors({ global: result.error });
            show({ message: result.error });
          }
        } else {
